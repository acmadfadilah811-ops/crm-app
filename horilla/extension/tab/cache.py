"""
Resolver/bootstrap cache for Tab view extensions (_inherit_tab).
"""
...
def clear_resolver_cache() -> None:
    """Clear per-tab-view-class resolution cache."""
    with RESOLVER_LOCK:
        RESOLVER_CACHE.clear()


def is_bootstrap_applied() -> bool:
    """Return whether tab extensions have been composed this process."""
    return _BOOTSTRAP_APPLIED[0]


def set_bootstrap_applied(applied: bool = True) -> None:
    """Record whether apply_tab_extensions has completed."""
    _BOOTSTRAP_APPLIED[0] = applied


def reset_bootstrap_applied() -> None:
    """Force apply_tab_extensions to recompose on next resolve."""
